    ...
    return (
        <section id={id} className={styles.section}>
            {isExpandable ? <h2 className={styles.heading}>
                <button className={styles.button} type="button" aria-expanded={isExpanded ? "true" : "false"} aria-controls={`${id}-content`} onClick={toggleIsExpanded}>{name} ({itemQuantity}) <span aria-hidden className={styles.arrow}><Arrow /></span></button>
            </h2>
            : 
            ...
            (<div id={`${id}-content`} hidden={!isExpanded}>
                {children}
            </div>)}
